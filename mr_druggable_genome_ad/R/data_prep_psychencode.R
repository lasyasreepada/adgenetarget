...
psychencode <- read_tsv(gzfile("eqtl_data_psychencode/significant.txt"), col_types=cols(.default="c"))

psychencode$new_gene_id <- gsub("\\..*", "", psychencode$gene_id)


# alt allele == effect allele
alleles <- read_tsv(gzfile("eqtl_data_psychencode/snps.txt"), col_types=cols(.default="c"))
...
psychencode_druggable <- subset(psychencode, psychencode$new_gene_id %in% druggable$ensembl_gene_id)
